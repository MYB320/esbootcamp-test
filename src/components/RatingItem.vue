<template>
  <div>
    <div class="flex mx-5 mb-2 bg-[#F9FAFB] border-gray-100 border rounded">
      <div class="w-20 h-20"><img src="../assets/doc.svg" alt="doctor" /></div>
      <div class="ml-4">
        <div>
          <h1 class="text-cyan-700 font-semibold">Dr. Amine Mohmaed</h1>
          <h3 class="text-gray-500 text-sm font-medium">1m ago</h3>
        </div>
        <div class="flex items-center">
          <h3 class="text-yellow-400 font-medium">5</h3>
          <svg
            class="w-4 h-4 text-yellow-400"
            fill="currentColor"
            viewBox="0 0 20 20"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"
            ></path>
          </svg>
        </div>
        <div class="text-sm">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Vitae dolorum
          consectetur unde modi commodi....
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: 'RatingItem',
}
</script>

<style></style>
